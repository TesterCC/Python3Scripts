...
class SohuBaseClass(object):
    ...
    def request_with_cookie(self):
        ...
        for cookie in Cookies:
            s.cookies.set(cookie['name'], cookie['value'])    # cookie持久化

        header = s.headers.update(headers)
        print(s.headers)

        self.s = s

        # 访问写文章界面
        response = s.get(self.edit_article_url)
        ...
        event_title = "第888届中国活动家大会"  # 通过函数获取
        ...
        req = s.post(self.publish_direct_url, data=post_data, headers=header)
        article_id = req.text
        print("Article id is {}".format(article_id))
        print("{0}".format(req.status_code))

    def check_publish_url(self):
        # Article Check
        """
