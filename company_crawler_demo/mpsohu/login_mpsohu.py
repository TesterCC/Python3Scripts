...
USERNAME = ""    # [18702895635, 15281005385, 18683715921, 18782291154, 18108061758]
...
def mpsohu_sender(url=LOGIN_URL, username=USERNAME, password=PASSWORD):
    ...
    if driver.current_url == after_login_page_url:
        ...
        # print("Start to test save draft.")
        # req = s.post(post_draft_url, data=post_data, headers=header)
        sleep(2)

        # print("Start to test modify draft.")
        print("Start to test directly send.")
        req = s.post(publish_direct_url, data=post_data, headers=header)
        print(req.text)
