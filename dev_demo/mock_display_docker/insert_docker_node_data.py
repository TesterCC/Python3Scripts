...
def write_node_heartbeat_time_by_tree():
    with open(config_file) as f:
        old_tree_json = json.load(f)
        known_nodes = old_tree_json.get('known_nodes')

        tree_json = old_tree_json

        tree_json["node_heartbeat_time"] = dict()
        for node in known_nodes:
            tree_json["node_heartbeat_time"][node.get('id')] = round(time.time() * 1000)

    with open(f'/opt/i_cm/controlled/tree.json', 'w') as f:
        f.seek(0)
        f.truncate()
...
if __name__ == '__main__':
    ...
    write_node_heartbeat_time_by_tree()
    print("[D] Finish update tree.json node_heartbeat_time")
