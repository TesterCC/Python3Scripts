...
#         os.makedirs(path)
#
#     for url in untrans_url_list:
#         print(url)
#         response = requests.get(url, verify=False)
#         # print(response.headers['Content-Type'].split('/')[-1])
#         if response.headers['Content-Type'].split('/')[-1] == 'pdf':   # exclude 404
#             filename = os.path.basename(url)
#             with open('{0}/{1}'.format(path, filename), 'wb') as f:
#                 f.write(response.content)
#
# download_pdf(untrans_url_list)
