...
def compute_checksum(data):
    '''
    这个Python函数的实现逻辑为计算给定数据的TCP校验和。
    具体实现细节如下：
    首先判断数据长度是否为奇数，如果是奇数则在数据后面添加一个字节的空字节（\x00），变成偶数长度。
    接下来将数据中每两个字节转换成一个16位整数，并将这些整数相加。
    如果相加后的结果超过了16位，则将超出的高位折叠到低16位，不断循环，直到不再超过16位。
    计算校验和：将前面得到的总和按位取反并与0xffff进行按位与（即保留16位），得到16位校验和。
    返回计算出的TCP校验和。
    总之，该函数用来验证给定数据在传输过程中是否被修改或损坏。如果得到的校验和与传输过来的校验和不一致，那么我们就可以判断这个数据包已经被篡改了。
    '''
    # padding
    if len(data) % 2 == 1:
        data += b'\x00'
...
def parse_packet(data):
    '''
    这个函数的逻辑如下：
    定义一个函数parse_packet，它接受一个字节数组data作为参数。

    创建一个名为packet的空字典。packet将包含从data中提取的网络包信息。

    首先对data进行Ethernet头部解析。计算出数据长度并使用struct模块按照特定格式解包数据，将结果存储在eth_tuple中。将源MAC地址、目的MAC地址、协议类型存储在packet中。

    检查协议类型是否为IPv4，如果不是则返回None。否则，继续进行IPv4头部解析。获取IPv4头部的长度、选项长度、数据长度，并使用struct模块按照特定格式解包数据，将结果存储在ip_tuple中。将协议类型、源IP地址、目的IP地址存储在packet中。

    检查协议类型是否为TCP，如果不是则返回None。否则，继续进行TCP头部解析。获取TCP头部的长度、选项长度、数据长度，并使用struct模块按照特定格式解包数据，将结果存储在tcp_tuple中。将源端口号、目的端口号、序列号、确认号、标志和TCP负载数据长度存储在packet中。

    返回packet字典。

    总体来说，这个函数的逻辑是解析数据包的各个部分，并将解析结果存储在一个字典中。函数使用了struct和socket模块来对字节数据进行解析和转换，从而提取出各种协议头部中的信息。最终返回的字典中包含了解析出的重要网络包信息。
    '''
    packet = {}
...
def main():
    fip = "192.168.100.2"
    fkip = "172.16.100.3"  # if cannot use, change bfkip to local ip
    reip = "192.168.100.6"
    ...
    send_fd = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_TCP)
